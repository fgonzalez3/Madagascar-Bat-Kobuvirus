...
#set wd
homewd = "/Users/flg9/Desktop/Developer/brook_lab/Madagascar-Bat-Kobuvirus/pipelines/BayesianTree/3_BuildTree/"
setwd(paste0(homewd))

#load the dataset and query
dat <- read.csv(file = "BEAST_Metadata.csv", header = T, stringsAsFactors = F)

head(dat)

#now, get the fasta file 
fasta.dat <- read.fasta(file= paste0(homewd,"Kobuvirus_MSA_Extraction.fasta"), forceDNAtolower = F, as.string = T)
...
dat$collection_date <- format(dat$collection_date, "%Y-%m-%d")
